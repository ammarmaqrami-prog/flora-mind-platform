...
export default function PerfumePage() {
  const [messages, setMessages] = useState<{ role: "user" | "assistant"; content: string; images?: string[]; isError?: boolean }[]>([
    { role: "assistant", content: "مرحباً! أنا مساعدك في تصميم العطور. أخبرني عن الروائح التي تفضلها (خشبي، زهري، شرقي، منعش...) أو ارفع صورة لعطر تحبه، وسأقترح لك مكونات طبيعية ونسب مزج دقيقة لصناعة عطرك الخاص خطوة بخطوة." },
  ]);
  ...
  const callGroqAPI = async (userMessage: string, images?: string[], isRetry: boolean = false) => {
    if (isBlocked) {
  ...
  const handleSendMessage = () => {
    ...
    callGroqAPI(input, selectedImages);
    setInput("");
    setSelectedImages([]);
  };
  ...
  const handleRetryLastMessage = () => {
    ...
    callGroqAPI(lastUserMsg.content, lastUserMsg.images, true);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-12">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-green-900">خدمة صناعة العطور</h1>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 md:p-8 shadow-md mb-10 border border-green-100">
          ...
          <p className="text-gray-700 leading-relaxed mb-4">
            <strong>المشكلة:</strong> كثير من الناس يحبون العطور لكن لا يعرفون كيف يصنعون عطراً خاصاً يعكس ذوقهم، فيعتمدون على العطور الجاهزة.
          </p>
          <p className="text-gray-700 leading-relaxed mb-4">
            <strong>كيف تعمل؟</strong> تخبرني عن الروائح التي تفضلها (خشبي، زهري، شرقي، منعش...)، أو ترفع صورة لعطر يعجبك. سأقترح مكونات طبيعية، وأحدد نسب المزج بدقة، وأعطيك طريقة التحضير خطوة بخطوة لصناعة عطرك الخاص.
          </p>
          <p className="text-gray-700 leading-relaxed mb-4">
            <strong>الفوائد:</strong> تجربة شخصية مميزة، تعلم أساسيات صناعة العطور، والوصول إلى تركيبة عطر فريدة.
          </p>
          <div className="mt-4 inline-block bg-green-100 text-green-900 px-6 py-2 rounded-full text-lg font-semibold">
            السعر: 3 ريال كل شهرين
          </div>
        </div>

        <div className="bg-gradient-to-br from-green-50 to-sky-50 rounded-2xl p-6 md:p-8 mb-12 border border-green-200">
          <h2 className="text-2xl font-bold text-green-900 mb-4">🌸 عائلات العطور الرئيسية</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            {[
              { name: "خشبي", icon: "🪵", examples: "عود، صندل، أرز" },
              { name: "زهري", icon: "🌹", examples: "ورد، ياسمين، زنبق" },
              { name: "شرقي", icon: "✨", examples: "عنبر، مسك، فانيليا" },
              { name: "منعش", icon: "🍋", examples: "ليمون، برغموت، نعناع" },
            ].map((fam, i) => (
              <div key={i} className="bg-white/70 p-4 rounded-xl">
                <div className="text-3xl mb-2">{fam.icon}</div>
                <h3 className="font-bold text-green-800">{fam.name}</h3>
                <p className="text-xs text-gray-600">{fam.examples}</p>
              </div>
            ))}
          </div>
          <p className="text-sm text-gray-500 mt-4">* أخبرني بالمكونات التي تعجبك لأصمم لك تركيبة متوازنة.</p>
        </div>

        {/* حالة الاستخدام المجاني */}
        {!user && (
          <div className="mb-4">
            {isBlocked ? (
              <div className="bg-red-50 border border-red-200 text-red-800 p-3 rounded-xl text-center">
                ...
                للاستمتاع بفترة تجريبية 14 يومًا.
              </div>
              ...
              <div className="bg-amber-50 border border-amber-200 text-amber-800 p-3 rounded-xl text-center">
                ...
                لاستخدام غير محدود لمدة 14 يومًا.
              </div>
            ) : null}
          </div>
        )}
        ...
        <div className="relative">
          ...
          <h3 className="text-xl font-bold text-green-800 mb-2 text-center">تحدث مع مصمم العطور</h3>
          <div className="bg-white rounded-2xl shadow-xl border border-green-200 overflow-hidden" ref={chatContainerRef}>
            ...
            <div className="border-t border-gray-200 p-4 flex items-center gap-2 bg-white">
              ...
              <button
                ...
                className={`p-3 rounded-full transition ${isRecording ? "bg-red-500 text-white" : "text-gray-600 hover:bg-green-50"}`}
              ...
              <input
                ...
                placeholder={isRecording ? "🎙️ جاري الاستماع..." : "مثلاً: أحب رائحة العود والورد..."}

...
export default function PredictiveAssistantPage() {
  const [messages, setMessages] = useState<{ role: "user" | "assistant"; content: string; images?: string[]; isError?: boolean }[]>([
    { role: "assistant", content: "مرحباً! أنا مساعدك الزراعي التنبؤي. أستطيع تحليل صور نباتاتك وبياناتك الزراعية للتنبؤ بالمشاكل قبل حدوثها. أرسل لي صورة لنباتك أو صف لي حالته، وسأخبرك بالمخاطر المحتملة والإجراءات الوقائية." },
  ]);
    ...
    try {
      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          message: "",
          images: [],
          featureName: "المساعد الزراعي التنبؤي",
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { localStorage.removeItem("token"); localStorage.removeItem("currentUser"); setUser(null); checkAccess(); }} className="text-red-600 text-sm hover:underline">خروج</button>
            </div>
          ) : (
            <button onClick={() => setIsLoginModalOpen(true)} className="text-green-800 hover:underline">تسجيل الدخول</button>
          )}
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-12">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-green-900">المساعد الزراعي التنبؤي</h1>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 md:p-8 shadow-md mb-10 border border-green-100">
          ...
          <p className="text-gray-700 leading-relaxed mb-4">
            <strong>المشكلة:</strong> المزارع غالباً يلاحظ مشكلة النبات بعد ما تتطور، مما يصعب إنقاذ المحصول.
          </p>
          <p className="text-gray-700 leading-relaxed mb-4">
            <strong>كيف تعمل؟</strong> خدمة ذكية تحلل صور النبات وبيانات الحالة لتتنبأ بالمشاكل قبل حدوثها، مع خطوات وقائية.
          </p>
          <p className="text-gray-700 leading-relaxed mb-4">
            <strong>الفوائد:</strong> تقليل خسائر المحاصيل، تدخل مبكر، رفع جودة الإنتاج.
          </p>
        ...
        <div className="bg-gradient-to-br from-green-50 to-sky-50 rounded-2xl p-6 md:p-8 mb-12 border border-green-200">
          <h2 className="text-2xl font-bold text-green-900 mb-4">⚠️ علامات الإنذار المبكر</h2>
          <ul className="space-y-3 text-gray-700">
            <li>🍂 اصفرار أطراف الأوراق → نقص نيتروجين</li>
            <li>💧 ذبول منتصف النهار → إجهاد مائي</li>
            <li>🐛 بقع شفافة → إصابة حشرية</li>
            <li>🌫️ طبقة بيضاء → بياض دقيقي</li>
          </ul>
        </div>
        ...
        <div className="relative">
          ...
          <h3 className="text-xl font-bold text-green-800 mb-2 text-center">تحدث مع المساعد التنبؤي</h3>
          <div className="bg-white rounded-2xl shadow-xl border border-green-200 overflow-hidden" ref={chatContainerRef}>
            ...
            <div className="border-t border-gray-200 p-4 flex items-center gap-2 bg-white">
              ...
              <button onClick={toggleVoiceRecording} className={`p-3 rounded-full ${isRecording ? "bg-red-500 text-white" : "text-gray-600 hover:bg-green-50"}`} title={isRecording ? "إيقاف" : "تسجيل"}>🎤</button>
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleSendMessage()}
                placeholder={isRecording ? "🎙️ جاري الاستماع..." : "صف حالة نباتك..."}
